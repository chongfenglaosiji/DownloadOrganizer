...
定义桌面通知（托盘气泡）的显示行为与开关控制：通知在常驻模式下随文件移动成功触发，受运行时开关控制；开关可通过系统托盘菜单与图形配置界面（`--gui`）操作，并持久化于配置项 `notify_on_move`。

## Requirements

### Requirement: 移动完成时显示桌面通知

系统 SHALL 在托盘/通知后端可用且常驻模式下，每次成功移动文件后显示桌面通知（托盘气泡）；`--once` 单次整理模式不显示移动通知。托盘/通知后端不可用（如 pystray 未安装、无显示环境、托盘启动失败）时静默降级，不显示桌面通知。通知显示与否受通知开关控制，配置项 `notify_on_move`（默认 `true`）决定程序启动时通知开关的初始状态。

#### Scenario: 常驻模式通知开启时移动成功显示通知
- **WHEN** 托盘/通知后端可用、常驻模式下通知开关为开，且一个文件被成功移动
- **THEN** 系统显示包含文件名的桌面通知
