...
系统 SHALL 在常驻模式下每次成功移动文件后显示桌面通知（托盘气泡）；`--once` 单次整理模式不显示移动通知。通知显示与否受通知开关控制，配置项 `notify_on_move`（默认 `true`）决定程序启动时通知开关的初始状态。

#### Scenario: 常驻模式通知开启时移动成功显示通知
- **WHEN** 常驻模式下通知开关为开，且一个文件被成功移动
- **THEN** 系统显示包含文件名的桌面通知

#### Scenario: 常驻模式通知关闭时移动成功不显示通知
- **WHEN** 常驻模式下通知开关为关，且一个文件被成功移动
- **THEN** 系统不显示桌面通知
...
系统 SHALL 在系统托盘菜单提供"通知"勾选开关，用户可随时切换通知开关状态；切换 SHALL 即时生效，无需重启程序、无需编辑配置文件。托盘菜单中该勾选项的状态 SHALL 反映当前运行时开关状态，并 SHALL 在启动时以配置项 `notify_on_move` 初始化。该开关为运行时状态，程序重启后 SHALL 以配置项 `notify_on_move` 重置（托盘切换不写回配置文件）。
...
#### Scenario: 重启后开关恢复为配置值
- **WHEN** 用户在托盘关闭通知后重启程序，且配置中 `notify_on_move = true`
- **THEN** 通知开关恢复为开
